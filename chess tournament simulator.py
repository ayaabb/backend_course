...
for j in range(3):
    print(f"Round {j + 1} :")
    for i in range(2):
        reversed_ratio = 1 - (players[players_per_round[c+i][0]].ranking / players[players_per_round[c+i][1]].ranking)
        print(f"Player {players[players_per_round[c+i][0]].name} vs Player {players[players_per_round[c+i][1]].name}\n"
